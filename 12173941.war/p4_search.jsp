...
Statement listmt = null;
   ...
   //3. 연결자를 생성한다.
    ...
    String sql = "SELECT * FROM post WHERE sort2=?"; 
    //5.쿼리를 전달하고, ResultSet 객체를 반환 받는다.
    PreparedStatement pstmt = con.prepareStatement(sql);

    pstmt.setString(1,sort2);
 ...
 <form action="#" method="get">
        ...
        <nav>
            <ul class="list">
				...
				<%
  				  while(rs.next()) {
				%><em><input type="button" value="<%=rs.getString("sort1") %>" style="background-color: white;" onclick="location.href='p4_sort2.jsp?sort1=<%=rs.getString("sort1") %>'"></em>
					<br>
				<%} %>
                ...
                <li>
                    ...
                    <em>
                        <select name="sort2_2">
                            <optgroup label="분류명_1을 선택하세요">
                            </optgroup>
                        </select>
                    </em>
                    <hr>
                    <br />
                    <em><button type="submit">검색</button></em>
                </li>
                <hr />
            </ul>
        </nav>
